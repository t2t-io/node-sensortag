...
var BATTERYVOLT_UUID                        = 'f000aae004514000b000000000000000';
...
var BATTERYVOLT_DATA_UUID                      = 'f000aae104514000b000000000000000';
var BATTERYVOLT_CONFIG_UUID                    = 'f000aae204514000b000000000000000';
var BATTERYVOLT_PERIOD_UUID                    = 'f000aae304514000b000000000000000';
...
function SensorTagCommon() {
  ...
  this.onBatteryVoltageBinded           = this.onBatteryVoltageChange.bind(this);
}
...
SensorTagCommon.prototype.writePeriodCharacteristic = function(serviceUuid, characteristicUuid, period, callback) {
  ...
  if(this.type === 'nrf51822')
  {
    console.log("Notify Period in:" +period);
    if(period<=1)
      period=1;
    else if(period>255)
    {
    ...
    console.log("Notify Period out:" +period);
  }
  else if(this.type === 'nrf51822_v2')
  {
    console.log("Notify Period in:" +period);
    
    period /= 1000; // input is scaled by units of 1000ms , Modify by Leon@20160616
    if(period<=1)
      period=1;
    else if(period>255)
    {
...
SensorTagCommon.prototype.setBatteryVoltagePeriod = function(period, callback) {
  this.writePeriodCharacteristic(BATTERYVOLT_UUID, BATTERYVOLT_PERIOD_UUID, period, callback);
};

SensorTagCommon.prototype.enableBatteryVoltage = function(callback) {
  this.enableConfigCharacteristic(BATTERYVOLT_UUID, BATTERYVOLT_CONFIG_UUID, callback);
};

SensorTagCommon.prototype.disableBatteryVoltage = function(callback) {
  this.disableConfigCharacteristic(BATTERYVOLT_UUID, BATTERYVOLT_CONFIG_UUID, callback);
};

SensorTagCommon.prototype.readBatteryVoltage = function(callback) {
  this.readDataCharacteristic(BATTERYVOLT_UUID, BATTERYVOLT_DATA_UUID, function(error, data) {
    if (error) {
      return callback(error);
    }

    this.convertBatteryVoltageData(data, function(voltagemv) {
      callback(null, voltagemv);
    });
  }.bind(this));
};

SensorTagCommon.prototype.onBatteryVoltageChange = function(data) {
  this.convertBatteryVoltageData(data, function(voltagemv) {
    this.emit('batteryVoltageChange', voltagemv);
  }.bind(this));
};

SensorTagCommon.prototype.notifyBatteryVoltage = function(callback) {
  this.notifyCharacteristic(BATTERYVOLT_UUID, BATTERYVOLT_DATA_UUID, true, this.onBatteryVoltageBinded, callback);
};

SensorTagCommon.prototype.unnotifyBatteryVoltage = function(callback) {
  this.notifyCharacteristic(BATTERYVOLT_UUID, BATTERYVOLT_DATA_UUID, false, this.onBatteryVoltageBinded, callback);
};
